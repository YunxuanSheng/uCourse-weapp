...
let TKK = Taro.getStorageSync('TKK')
...
function updateTKK() {
  return new Promise(function(resolve, reject) {
    ...
    if (Number(Taro.TKK.split('.')[0]) === now) {
      resolve()
    } else {
      Taro.request({ url: 'https://translate.google.cn' })
        .then(function(res) {
          var code
          try {
            if (res.data.includes('var')) {
            // strategy 1
            code = res.data
              .match(/tkk:'.*\(\)\)/g)[0]
              .replace("tkk:'", '')
            ...
            // strategy 2: plain tkk
            code = res.data
              .match(/tkk:'.*',experiment/g)[0]
              .replace("tkk:'", '')
              .replace("',", '')
              .replace('experiment', '')
            TKK = code
