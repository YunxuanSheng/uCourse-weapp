...
function parse(md, options) {
      ...
      inlineToken.children &&
        inlineToken.children.forEach(function(token, index) {
          if (['text', 'code'].indexOf(token.type) > -1) {
            ret.push({
              type: env || token.type,
              content: pangu.spacing(token.content),
              data: tokenData,
            })
            env = ''
            tokenData = {}
            ...
            if (options.link) {
              ...
              if (!token.href.includes('.')) {
                // 提供页面链接的简写形式，可以使用 pageName#ID 的方式访问
                // 比如 'eval#30' === '/subpackages/eval/pages/eval/eval?id=30'
                // 向下兼容到保留原用法
                env = 'internal_link'
                const tempHrefArr = token.href.split('#')
                const pageName = tempHrefArr[0]
                const id = tempHrefArr[1]
                if (['course', 'course-detail'].some(v => v === pageName)) {
                  href = `/pages/${pageName}/${pageName}?code=${id}`
                } else {
                  href = `/pages/${pageName}/${pageName}?id=${id}`
                }
                if (pageName === 'eseval') {
                  href = `/subpackage/exchange/pages/eval/eval?id=${id}`
                }
              } else {
                env = 'external_link'
                href = token.href
  ...
  var getBlockContent = function(blockToken, index, firstInLi) {
    if (blockToken.type === 'htmlblock') {
      ...
      if (blockToken.params === 'html' || blockToken.params === 'markdown') {
