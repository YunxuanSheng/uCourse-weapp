...
export default class Painter {
  ...
  _doClip(borderRadius, width, height) {
    if (borderRadius && width && height) {
      ...
      if (
        !(
          this.getSystemInfo() &&
          this.getSystemInfo().version <= '6.6.6' &&
          this.getSystemInfo().platform === 'ios'
        )
      ) {
        this.ctx.clip()
      }
  ...
  _preProcess(view, notClip) {
    ...
    switch (view.type) {
      case 'text': {
        ...
        this.ctx.font = `normal ${fontWeight} ${view.css.fontSize.toPx()}px ${
      ...
      case 'image': {
        ...
        const ratio = this.getSystemInfo().pixelRatio
          ? this.getSystemInfo().pixelRatio
          : 2
    ...
    if (view.css && view.css.right) {
      if (typeof view.css.right === 'string') {
        ...
        x =
          this.style.width -
          rights[0].toPx(true) -
          this.globalTextWidth[rights[1]] * (rights[2] || 1)
      }
      ...
      if (typeof view.css.left === 'string') {
        ...
        x =
          lefts[0].toPx(true) + this.globalTextWidth[lefts[1]] * (lefts[2] || 1)
      }
    } else {
      x = 0
    }
    const y =
      view.css && view.css.bottom
        ? this.style.height - height - view.css.bottom.toPx(true)
        : view.css && view.css.top
          ? view.css.top.toPx(true)
          : 0
  ...
  _fillAbsText(view) {
    ...
    if (view.id) {
      ...
      for (let i = 0; i < textArray.length; ++i) {
        textWidth =
          this.ctx.measureText(textArray[i]).width > textWidth
            ? this.ctx.measureText(textArray[i]).width
            : textWidth
      }
      this.globalTextWidth[view.id] = width
        ? textWidth < width
          ? textWidth
          : width
        : textWidth
