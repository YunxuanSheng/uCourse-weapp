...
export default class EvalContent extends Component {
  ...
  translateContent = async content => {
    ...
    try {
      ...
      Taro.hideLoading()
      Taro.showToast({
        title: Taro.T._('Translating failed'),
        icon: 'none',
      })
    }
  }
  ...
  promptMdLink = async () => {
    ...
    const { confirm } = await Taro.showModal({
      title: '评测 ID',
      content: `你可以在评测中通过\n[链接文字](eseval#${this.props.theEval.id})\n的语法来链接到这条评测`,
      confirmColor: '#ff9800',
      cancelText: '取消',
      confirmText: '复制链接',
    })
  ...
  render() {
    const { score } = this.props.theEval
        ...
        <View className="eval-rate">
          <View className="left">
            <View className="rate-container">
              ...
              <Rate defaultValue={score} disabled />
