...
export default class BottomBar extends Component {
  ...
  handleFavEval = () => {
    if (this.state.isFaved) {
      ...
      Taro.showToast({ title: Taro.T._('Cancel favorite'), icon: 'none' })
      ...
      Taro.showToast({ title: Taro.T._('Favorited'), icon: 'none' })
  ...
  handleVoteTask = () => {
    ...
    if (type === 'pro' || type === 'con') {
      ...
      if (!this.props.myVote) {
        // new
        api.Exchanges.Votes.create({ evalId: evalId, type: voteTypeNum })
      } else if (this.props.myVote.type !== voteTypeNum) {
        // update
        api.Exchanges.Votes.update(this.props.myVote.id, { type: voteTypeNum })
      }
    } else if (type === 'delete' && !!this.props.myVote) {
      // delete
      api.Exchanges.Votes.delete(this.props.myVote.id)
    }
  ...
  navToEvalNew = () => {
    ...
    const mode = 'edit'
    Taro.navigateTo({
      url: `/subpackages/exchange/pages/school-eval-new/school-eval-new?id=${evalId}&mode=${mode}`,
    })
  }
  ...
  handleShowMore = async () => {
    ...
    if (tapIndex === 0) {
      ...
      Taro.showToast({ title: Taro.T._('Reported'), icon: 'none' })
    }
  }

  handleShowTrans = () => {
    // this.props.onShowTrans()
    this.setState(prevState => ({ isTranslated: !prevState.isTranslated }))
    Taro.eventCenter.trigger('showTrans')
  }
