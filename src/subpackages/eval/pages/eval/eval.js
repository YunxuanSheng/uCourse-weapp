...
export default class Eval extends Component {
  ...
  onShareAppMessage() {
    ...
    return {
      ...
      path: `/subpackages/eval/pages/eval/eval?title=${title}&id=${id}`,
    }
  }
  ...
  handleAskForPalette = async () => {
    ...
    const url = await api.Qr.get({
      page: 'subpackages/eval/pages/eval/eval',
  ...
  initRedirectNav = () => {
    ...
    const notCourseParent =
      pages.length > 1 &&
      pages[pages.length - 2].route !== 'pages/course/course'
  ...
  navToCourse = () => {
    if (this.state.showRedirectNav) {
      const { title, code, school_code } = this.state.theEval.course
      Taro.redirectTo({
        url: `/subpackages/course/pages/course/course?code=${code}&title=${title}&school_code=${school_code}`,
      })
    }
  }
      ...
      <FormIdReporter>
        <View className="eval-page">
          <Navigation align="left">
            <View className="nav" onClick={this.navToCourse}>
          ...
          {this.state.isLoading ? (
            ...
            <View>
              ...
              <UAd unitId="adunit-2c06fbbc2f853526" />
